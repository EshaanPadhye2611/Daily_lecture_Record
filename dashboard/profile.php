...
if(isset($_POST["submit_btn"])){
    // Fetching and sanitizing input data
    ...
    if(strlen($course_no) > 8){
        $_SESSION['alert'] = 'Course number too long';
        $_SESSION['alert_type'] = 'error';
    } else {
        // SQL query to insert data
        $sql = "INSERT INTO lecture_record (section_name, course_no, course_name, instructor_id, instructor_name, time, date, day) VALUES ('$section_name', '$course_no', '$course_name', '$instructor_id', '$instructor_name', '$time', '$date', '$day')";
        $query = mysqli_query($conn, $sql);
        if($query){
            $_SESSION['alert'] = 'Data has been inserted successfully!';
...
button[type="submit"], button[type="button"] {
    ...
    font-size: 16px;
    margin-top: 10px;
    transition: transform 0.6s; /* Adding transition */
}

button[type="submit"]:hover, button[type="button"]:hover {
    background-color: #0056b3;
    transform: scale(1.08); /* Scale transformation on hover */
}
...
<form  action="" method="POST">
    ...
    <a href="timetable.php?instructor_id=<?php echo $user['unique_id']; ?>">
        <button name="DLR_btn" type="button">Enter DLR</button>
    </a>
    <a href="logout.php">
        <button name="logout_btn" type="button">Logout</button>
    </a>
</form> 
